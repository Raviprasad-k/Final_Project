...
public class UsedCarsPage {
    ...
    public void selectCityChennaiIfModal() {
        try {
            if (cityInput.isDisplayed()) {
                ...
                Thread.sleep(500);
                try { chennaiSuggestion.click(); } catch (Exception e) { cityInput.sendKeys(Keys.ENTER); }
    ...
    public List<List<String>> scrapeUsedCars() {
        ...
        // We find the fresh list inside the loop to avoid StaleElementReferenceException
        for (int i = 0; i < 6; i++) {
            try {
                String name = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(carNamesLoc)).get(i).getText();
                String price = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(carPricesLoc)).get(i).getText();
                String fuel = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(fuelTypesLoc)).get(i).getText();
